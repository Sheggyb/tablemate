...
export default function ChartCanvas({
  ...
  const filteredUnseated = unseatedGuests.filter(g => {
    ...
    return `${g.first_name} ${g.last_name}`.toLowerCase().includes(guestSearch.toLowerCase());
  });
  ...
  const onTableContextMenu = (e: React.MouseEvent, id: string) => {
    e.preventDefault();
    e.stopPropagation();
    setSelected(id);
  ...
  const ctxTable = ctxMenu ? tables.find(t => t.id === ctxMenu.tableId) : null;
        ...
        <div ref={canvasRef}
          ...
          <div style={{ transform: `translate(${offset.x}px,${offset.y}px) scale(${scale})`, transformOrigin: "0 0", position: "absolute", top: 0, left: 0 }}>
            {tables.map(table => {
              ...
              return (
                <div key={table.id} data-table={table.id}
                  style={{
