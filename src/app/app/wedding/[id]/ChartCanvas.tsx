          ...
          {viewMode === "list" && (
            <div className="flex-1 overflow-hidden flex min-h-0">
              ...
              {selected && (() => {
                ...
                return (
                  <div className="w-64 flex-shrink-0 flex flex-col border-l animate-slide-in overflow-y-auto"
                    style={{ background: cs.surface, borderColor: cs.border }}>
                    <div className="px-4 py-3 border-b" style={{ borderColor: cs.border }}>
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-xs mt-0.5" style={{ color: cs.textMuted }}>{tg.length}/{table.capacity} seated · {vacant} vacant</p>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-0.5">
                      {tg.length === 0 && <div className="text-center py-8"><p className="text-xs" style={{ color: cs.textMuted }}>Drag guests here to seat them</p></div>}
                      {tg.map(g => (
                        <div key={g.id} className="flex items-center gap-2 p-2 rounded-lg group" style={{ background: "transparent" }}
                          onMouseEnter={e => (e.currentTarget.style.background = cs.surface2)}
                          onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                          <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: groupColor(g.group_id) }}/>
                          <span className="text-xs flex-1 truncate" style={{ color: cs.text }}>{g.first_name} {g.last_name}</span>
                          <span className="text-xs">{MEAL_ICON[g.meal || "standard"]}</span>
                          <button onClick={() => onSeatGuest(g.id, null, null)} className="opacity-0 group-hover:opacity-100 text-xs hover:opacity-70" style={{ color: "var(--danger)" }}>✕</button>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="px-3 pb-3 space-y-2">
                      <button onClick={() => { if (confirm(`Remove all guests from "${table.name}"?`)) tg.forEach(g => onSeatGuest(g.id, null, null)); }}
                        ...
                        style={{ border: `1px solid var(--danger)`, color: "var(--danger)" }}>Clear table</button>
                      <button onClick={() => { if (confirm(`Delete "${table.name}"?`)) { onDeleteTable(table.id); setSelected(null); } }}
                        ...
                        style={{ background: "var(--danger)" }}>🗑 Delete table</button>
                    </div>
                  </div>
                );
              })()}
