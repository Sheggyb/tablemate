...
export default function PlannerClient({
  ...
  const addTable = useCallback(async (name: string, shape: "round" | "rectangle" | "oval", capacity: number) => {
    ...
    if (isDemo && activeTables.length >= 4) { showToast("Demo limited to 4 tables — sign up for unlimited!", "error"); return; }
    ...
    <div className="flex flex-col h-screen overflow-hidden" style={{ background: cs.bg, color: cs.text }}>
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="rounded-2xl p-6 w-full max-w-sm shadow-2xl animate-fade-in max-h-[90vh] overflow-y-auto"
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={() => {
                ...
                if (state.venues[0] && !isDemo) {
                  // menu items are saved individually
                }
